...
const { round2, frDate, iso } = require('./util');

const EMPTY = { meta: { imports: [] }, balances: {}, cash: {}, positions: [], operations: [] };

const load = () => store.read('portfolio.json', EMPTY);
const save = p => store.write('portfolio.json', p);
...
function mergePositions(portfolio, positions) {
  ...
  for (const p of positions) {
    // livrets / fonds sans quantité : traités comme un solde
  ...
  // lignes absentes de l'instantané → soldées
 ...
 * @param {object} [portfolio] instantané déjà chargé
 * @param {{live?:boolean}} opts live=false → dernier cours connu, sans appel réseau
 */
async function value(portfolio = load(), opts = {}) {
...
/* ============================================================
   Historique (un point par jour, alimenté à chaque valorisation)
   ============================================================ */

function pushHistory(snapshot) {
  let hist = store.read('history.json', []);
  if (!Array.isArray(hist)) {                     // fichier corrompu → on repart d'une série vide
    console.error('[history] history.json n\'est pas un tableau, réinitialisé');
    hist = [];
  }
  ...
  const point = { date: today, val: snapshot.totals.patrimoine, cap: snapshot.totals.capital, comptes };
  const last = hist[hist.length - 1];
  if (last && last.date === today) hist[hist.length - 1] = point;
  else hist.push(point);
  store.write('history.json', hist);
  return hist;
}

const history = () => store.read('history.json', []);

module.exports = { load, save, value, mergeOperations, mergePositions, pushHistory, history, classOf, EMPTY };
